...
const EMPTY_USER: ProfileViewUser = {
    suid: "-",
    username: "-",
    fullName: "User",
    birthDate: "-",
...
const InfoRow: React.FC<{ icon: React.ReactNode; label: string; value: string; theme: boolean }> = ({
    ...
    <div
        className={`group flex items-center gap-5 rounded-2xl border px-5 py-4 transition-all duration-300 shadow-sm cursor-default ${theme
    ...
    >
        <span
            className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl shadow-sm transition-all duration-300 ${theme
        ...
        <span className="flex min-w-0 flex-col gap-0.5">
            <span className={`text-xs font-bold uppercase tracking-wider transition-colors duration-300 ${theme ? "text-gray-400 group-hover:text-blue-400" : "text-neutral-500 group-hover:text-red-500"
                }`}>
                {label}
            </span>
            <span className={`truncate text-base font-extrabold transition-colors duration-300 ${theme ? "text-gray-100 group-hover:text-white" : "text-neutral-900 group-hover:text-neutral-950"
                }`}>
                {value}
            </span>
        </span>
...
const ProfileView: React.FC<ProfileViewProps> = ({
    ...
    const [userData, setUserData] = useState<ProfileViewUser>(user || EMPTY_USER);
    const [isImageModalOpen, setIsImageModalOpen] = useState(false);

    // 🎯 FIX: "adminData" nahi, "user" key vaparo (login pachi real data ahi save thay che)
    useEffect(() => {
        if (user) {
        ...
        if (stored) {
            try {
                ...
                setUserData({
                    suid: String(parsed.suid ?? "-"),
                    username: parsed.username ?? "-",
                    fullName: parsed.name ?? "User",
                    birthDate: parsed.bod ?? "-",
                    joinedDate: parsed.joiningDate ?? "-",
                    roleLabel: parsed.roleName ?? parsed.roleCode ?? "USER",
                    avatarUrl: parsed.avatar || undefined,
                });
            } catch (e) {
    ...
    return (
        <div
            className={`w-full h-full p-4 sm:p-8 transition-all duration-300 ${theme ? "bg-[#0B1120] text-white" : "bg-white text-neutral-900"
                }`}
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <div className={`lg:col-span-4 flex flex-col items-center text-center p-6 rounded-3xl border transition-all duration-300 h-max group ${theme
                    ...
                    <div
                        ...
                        className={`flex h-32 w-32 items-center justify-center overflow-hidden rounded-full text-5xl font-black shadow-xl ring-4 cursor-pointer transition-all duration-300 ${theme
                    ...
                    <span
                        className={`mt-5 inline-flex items-center gap-2 rounded-full px-5 py-2 text-xs font-black tracking-widest uppercase shadow-sm transition-all duration-300 ${theme
                ...
                <div className="lg:col-span-8 space-y-6">
                    <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-3 rounded-xl px-8 py-3.5 text-base font-bold text-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02] hover:shadow-xl cursor-pointer w-fit ${theme
            ...
            {isImageModalOpen && (
                ...
                >
                    <div
                        ...
                        className={`relative max-w-md w-full aspect-square rounded-3xl p-3 border transition-all duration-300 scale-in-95 animate-in zoom-in-95 ${theme
                    ...
                    >
                        <button
                            onClick={() => setIsImageModalOpen(false)}
                            className={`absolute top-5 right-5 p-2 rounded-xl border transition-all duration-300 cursor-pointer hover:rotate-90 z-10 ${theme
                        ...
                        <div className="w-full h-full rounded-2xl overflow-hidden flex items-center justify-center">
                            {userData.avatarUrl ? (
                                ...
                                <div className={`w-full h-full flex items-center justify-center text-7xl font-black rounded-2xl ${theme ? "bg-gray-900/50 text-blue-400" : "bg-neutral-50 text-red-600"
                                    }`}>
                                    {initial}
                                </div>
                            )}
