...
interface SectionData {
    id: number;
    sectionName: string;
    sectionHead: number;       // Section Head User ID
    sectionHeadName: string;   // Section Head Name
...
export default function SectionList() {
    ...
    const fetchSections = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${API_URL}/sections`); // Ensure backend route matches
    ...
    const handleEditSection = (id: number) => {
        ...
        // Navigate to edit route or open modal here
    };
    ...
    const columns = [
        {
            ...
            accessor: (sec: SectionData) => sec.id,
        },
        {
            header: "Section Name",
            className: "text-left font-bold",
            accessor: (sec: SectionData) => sec.sectionName,
        },
        {
            header: "Section Head",
            className: "text-left",
            accessor: (sec: SectionData) => (
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto mt-2 sm:mt-1">
                    ...
                    <div className="relative group">
                        <button
                            ...
                            className={`flex items-center justify-between pl-9 pr-3 py-2.5 w-40 sm:w-44 rounded-xl border text-sm font-medium outline-none transition-all duration-300 ${
                                theme
                        ...
                        {isFilterOpen && (
                            <div className={`absolute right-0 z-20 mt-2 w-48 rounded-xl border py-1.5 shadow-xl backdrop-blur-md transform transition-all duration-200 origin-top-right ${
                                theme
