...
// 🎯 Backend nu QuoteData ne frontend na ActivityData ma convert karo
function mapQuoteToActivity(q: QuoteData): ActivityData {
    return {
        id: q.id,
        image: q.image_url,
        description: q.description || "",
        date: q.event_date.split("T")[0], // date-only, timezone tail hoy to kadhi nakho
    };
}
...
const Activities: React.FC = () => {
    ...
    // 🎯 FIX: Backend thi activities fetch karo (localStorage nahi have)
    useEffect(() => {
        fetchActivities();
    }, []);
    ...
    const handleSave = async () => {
        ...
        try {
            setSaving(true);
            const result = await quoteService.createQuote("activity", selectedImageFile, headerDate, description.trim());

            if (result.success) {
                setDataList((prev) => [mapQuoteToActivity(result.data), ...prev]);
                    ...
                    <div className="flex flex-wrap items-start gap-4">
                        ...
                        <div className="flex justify-end w-full mt-4">
                            <button
                                onClick={handleSave}
                                disabled={saving}
                                className={`w-full sm:w-auto px-8 h-12 text-white rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg disabled:opacity-60 ${theme ? "bg-blue-600 hover:bg-blue-500 shadow-blue-500/20" : "bg-red-600 hover:bg-red-700 shadow-red-500/30"}`}>
                                {saving ? (
                                    <span className="w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin" />
                                ) : (
                                    <Save size={18} />
                                )}
                                <span className="text-md font-bold tracking-wide">{saving ? "Saving..." : "Save Activity"}</span>
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="space-y-3 overflow-y-auto pr-1 grow">
                        {loading ? (
                            <div className="flex justify-center py-10">
                                <div className={`w-8 h-8 rounded-full border-4 border-t-transparent animate-spin ${theme ? "border-blue-500" : "border-red-600"}`} />
                            </div>
