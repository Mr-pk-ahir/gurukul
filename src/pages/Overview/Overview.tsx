...
export default function Overview() {
    ...
    return (
        <div className="h-screen w-full overflow-y-scroll snap-y snap-mandatory scroll-smooth bg-white [&::-webkit-scrollbar]:none [-ms-overflow-style:none] scrollbar-none">
            ...
            <section
                className={`h-screen w-full snap-start relative flex flex-col justify-between p-6 overflow-hidden transition-colors duration-500 ${theme ? "bg-slate-950" : "bg-white"
                    }`}
            >
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className={`flex flex-col justify-center items-center gap-1 w-10 h-10 bg-white rounded-xl shadow-md border transition-all active:scale-90 cursor-pointer ${theme ? "border-red-200" : "border-blue-200"
                                    }`}
                            >
                            ...
                            <div className={`absolute top-12 left-0 w-56 bg-white rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 origin-top-left z-50 ${theme ? "border border-red-200" : "border border-blue-200"
            ...
            <section
                className={`relative h-screen w-full snap-start flex items-center gap-10 justify-center text-center px-6 transition-colors duration-500 ${theme ? "bg-slate-950" : "bg-white"
                    }`}
            >
                <div className="relative z-20">
                    <Reveal>
                        <h1
                            className={`text-4xl md:text-5xl font-black mb-4 tracking-wide transition-colors ${theme ? "text-red-600" : "text-blue-600"
                                }`}
                        >
                            {section2.title}
                        </h1>
                ...
                <div className={`w-200 h-150 p-5 rounded-[50px] bg-amber-300`}>
                    <div className={`w-full h-full rounded-[40px] bg-gray-900`}>
            ...
            <section
                className={`h-screen w-full snap-start relative flex items-center justify-center transition-colors duration-500 ${theme ? "bg-slate-950" : "bg-white"
                    }`}
            >
                <Reveal
                    className={`relative z-10 text-center px-6 max-w-2xl p-6 rounded-3xl shadow-xl border ${theme ? "bg-slate-900/80 border-slate-800" : "bg-white/80 border-gray-100"
                        }`}
                >
